...
ggraph(grafo, layout = "kk") + 
  geom_edge_parallel(aes(color = edge, start_cap = label_rect(node1.name),
    end_cap = label_rect(node2.name)), arrow = arrow(length = unit(1, "mm")), 
    start_cap = circle(4, "mm"), end_cap = circle(4, "mm"), show.legend = FALSE) +
  geom_node_point(aes(colour = Community, size = Popularity), show.legend = FALSE) + 
  geom_node_label(aes(label = Community), show.legend = FALSE, repel = TRUE, alpha = 0.75)
```
...
A biblioteca [networkD3](https://github.com/christophergandrud/networkD3) usa uma estrutura de dados parecida, mas com dataframes separados para edges e nodes.
...
A última biblioteca, [visNetwork](http://datastorm-open.github.io/visNetwork/), oferece a possibilidade de exibir as definições completas ao interagir com cada nó.
...
Apesar de estar disponível em diferentes idiomas, o português não era uma das contempladas pelo _Multilingual Data Stewardship Terminology_. Seria possível, porém, cruzar dados de fontes compatíveis que possibilitariam um novo estudo de relações observando variações entre os idiomas.
